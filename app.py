...
if "user" in st.query_params:
    ...
    if auto_user and not st.session_state.get('logged_in', False):
        ...
        # Verify role from Cloud Database to restore permissions
        try:
...
if not st.session_state['logged_in']:
    ...
    with tab2:
        st.info("Recovery Word is required for password resets.")
        r_user = st.text_input("Set Student ID", key="reg_u")
        r_pwd = st.text_input("Set Password", type="password", key="reg_p")
        r_rec = st.text_input("Set Recovery Word", type="password", key="reg_rec")
            ...
            if r_user and r_pwd and r_rec:
                try:
                    supabase.table("users").insert({
                        "student_id": r_user, 
                        "password_hash": make_hashes(r_pwd), 
                        "recovery_word_hash": make_hashes(r_rec),
                        "role": "Student"
                    }).execute()
...
# ==========================================
# 2. Main System Logic (Real-time Accuracy Calculation)
# ==========================================

@st.cache_resource
def load_and_validate_model():
    try:
        # Load pre-trained pkl files from root
        ...
        # Calculate LIVE accuracy using your dataset
        ...
        # Live inference for real-time reporting
        y_pred = model.predict(X)
        live_accuracy = accuracy_score(y_true, y_pred)
        cm = confusion_matrix(y_true, y_pred)
        
        return model, le, live_accuracy, live_accuracy * 0.95, cm, feature_names
    except Exception as e:
        st.error(f"🚨 Setup Error: Model or CSV files missing in root. {e}")
...
if page == "🏠 Home":
    ...
    - **🤖 Predictive AI:** Calculate your stress risk instantly.
    - **💬 Generative AI:** Empathetic support powered by **Google Gemini**.
    - **📜 Cloud Storage:** Securely sync your assessment history.
    """)
    if test_acc > 0:
        col1, col2 = st.columns(2)
        col1.info(f"🎓 Training Accuracy (Real-time): {train_acc*100:.2f}%")
        col2.success(f"🧪 Testing Accuracy (Estimated): {test_acc*100:.2f}%")
    ...
    with c1:
        ...
        st.caption("💡 Dual Input: Drag sliders or type numbers directly.")

        # Sync logic for dual inputs
        ...
        if total > 24:
            st.error("Error: Input hours cannot exceed 24 hours.")
        else:
            if st.button("Analyze Stress Level", use_container_width=True):
                if model:
                    ...
                    ai_advice = "Analyzing..."
                    if gemini_model:
                        p = f"Professional University Counselor. Student: Study {study}h, Sleep {sleep}h, GPA {gpa}. Stress: {pred_label} ({confidence:.1f}%). Concise tips."
    ...
    with c2:
        ...
        if 'last_pred' in st.session_state:
            ...
            if st.session_state['user_role'] != "Guest":
                st.write("Does this assessment accurately reflect your state?")
                ...
                if st.button("❌ Inaccurate, Correct it"): st.session_state['feedback_mode'] = True

                if st.session_state.get('feedback_mode'):
                    with st.form("corr_form"):
                        corr = st.selectbox("Actual Stress Level", ["Low", "Moderate", "High"])
    ...
    with st.form("lifestyle_survey"):
        s_study = st.number_input("Average Study Hours", 0.0, 24.0, 5.0)
        s_sleep = st.number_input("Average Sleep Hours", 0.0, 24.0, 7.0)
        s_gpa = st.number_input("Current GPA", 0.0, 4.0, 3.0)
        s_stress = st.selectbox("Self-Assessed Stress", ["Low", "Moderate", "High"])
        if st.form_submit_button("Sync to Cloud"):
            supabase.table("user_feedback").insert({"student_id": st.session_state['username'], "Study_Hours_Per_Day": s_study, "Sleep_Hours_Per_Day": s_sleep, "GPA": s_gpa, "Stress_Level": s_stress}).execute()
            st.success("Survey synced.")
    ...
    with t1:
        ...
        except: st.error("Baseline CSV missing from root.")
    with t2:
    ...
    st.info("System utilizing Offline Batch Architecture. To update the AI core, redeploy updated .pkl files.")
